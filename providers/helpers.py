...
async def _save_last_read(ctx, message_id: str, subject: str, sender: str,
    ...
    """Persist the last-read watermark per user (upsert pattern)."""
    try:
        ...
        if page.items:
            await ctx.store.update("mail_last_read", page.items[0].id, data)
        else:
...
async def _all_accounts(ctx: Context) -> list[dict]:
    ...
    return [{"doc_id": d.id, **d.data} for d in page.items]
